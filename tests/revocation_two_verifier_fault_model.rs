...
use fiducia_auth::gate::{Authorization, RevocationAuthority, RevocationGate, DEFAULT_REFRESH_TIMEOUT};
...
fn verifier(authority: ReferenceAuthority) -> RevocationGate<ReferenceAuthority> {
    RevocationGate::new(RevocationCache::new(B, 1024), authority, DEFAULT_REFRESH_TIMEOUT)
}
...
async fn cold_start_fails_closed_on_both_instances() {
    ...
    assert!(matches!(a.authorize(&token, 100).await, Authorization::Unavailable(_)));
    assert!(matches!(b.authorize(&token, 100).await, Authorization::Unavailable(_)));
}
...
async fn revoke_propagates_to_one_instance_before_the_other_then_converges() {
    ...
    assert_eq!(a.authorize(&token, 100 + B + 1).await, Authorization::Revoked);
    ...
    assert_eq!(b.authorize(&token, 100 + B + 1).await, Authorization::Revoked);
}
...
async fn lift_converges_only_after_refresh() {
    ...
    assert_eq!(v.authorize(&token, 100 + B + 1).await, Authorization::Allowed);
}
...
async fn token_isolation() {
    ...
    assert_eq!(v.authorize(&claims("tok-a", "t1"), 100).await, Authorization::Revoked);
    assert_eq!(v.authorize(&claims("tok-b", "t1"), 100).await, Authorization::Allowed);
    // A second tenant's distinct token is likewise unaffected.
    assert_eq!(v.authorize(&claims("tok-c", "t2"), 100).await, Authorization::Allowed);
}
