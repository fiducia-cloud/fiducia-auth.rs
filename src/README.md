...
- `keys.rs` — org-scoped API-key create/list/rotate/revoke and fail-closed
  durable `introspect`; stores only a hash of the secret (data plane).
...
replicas cannot silently lose org-index entries or resurrect revoked keys.
